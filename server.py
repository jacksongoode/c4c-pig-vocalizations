...
def get_context_mapping():
    ...
    if os.path.exists(mapping_file):
        ...
        pd.DataFrame(list(mapping.items()), columns=['id','context']).to_csv(mapping_file, index=False)
    return mapping
...
def upload():
    ...
    CHECKPOINT_PATH_VAL = os.path.join(os.getcwd(), 'checkpoints', 'Valence', 'Iter1', 'model_checkpoint_best.pt')
    CHECKPOINT_PATH_CON = os.path.join(os.getcwd(), 'checkpoints', 'Context', 'Iter1', 'model_checkpoint_best.pt')
    ...
    if os.path.exists(CHECKPOINT_PATH_VAL):
        try:
            model_val = load_resnet50_model(2, CHECKPOINT_PATH_VAL)  # Change 2 to appropriate number of classes
    ...
    if os.path.exists(CHECKPOINT_PATH_CON):
        try:
            model_con = load_resnet50_model(18, CHECKPOINT_PATH_CON)  # Change 18 to appropriate number of classes
    ...
    if model_val is not None:
        with torch.no_grad():
            ...
            print("Raw valence prediction values:", val_prediction.cpu().numpy())  # Debug: print raw output
    ...
    if model_con is not None:
        with torch.no_grad():
            ...
            print("Raw context prediction values:", con_prediction.cpu().numpy())  # Debug: print raw output
